...
{
    /**
     * Display a listing of Pemasukan documents (KD_DOK = 1)
     */
    public function index(Request $request)
    {
        ...
        // Calculate Stats
        $stats = [
            'totalDocuments' => Document::where('kd_dok', '1')->count(),
            'totalVolume' => Tangki::whereHas('document', fn($q) => $q->where('kd_dok', '1'))->sum('jml_satuan'),
            'totalTangki' => Tangki::whereHas('document', fn($q) => $q->where('kd_dok', '1'))->count(),
            'sentToHost' => Document::where('kd_dok', '1')->where('sent_to_host', true)->count(),
        ];
    ...
    /**
     * Preview Excel Upload before saving
     */
    public function importPreview(Request $request)
    {
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                if ($kdDok === '3') {
                    continue; // Skip Pengeluaran rows in Pemasukan import
                }

                $noBlAwb = $this->getRowValue($dataRow, ['NOBLAWB', 'NOBL', 'BLAWB', 'BL', 'NOAWB', 'BLNUMBER'], 8);
                $noBc11 = $this->getRowValue($dataRow, ['NOBC11', 'BC11', 'NOBC'], 12);
                $noDokInout = $this->getRowValue($dataRow, ['NODOKINOUT', 'NODOK', 'DOKINOUT', 'NOINOUT'], 19);
                $noTangki = $this->getRowValue($dataRow, ['NOTANGKI', 'TANGKI', 'NOMORTANGKI', 'TANKNO', 'CONTAINER', 'NOKONTAINER'], 15);
                ...
                $groupedData[$groupKey]['details'][] = [
                    ...
                    'no_bl_awb' => $noBlAwb ?: '-',
                    'tgl_bl_awb' => $this->parseDate($this->getRowValue($dataRow, ['TGLBLAWB', 'TGL_BL_AWB'], 9)),
                    'id_consignee' => $this->getRowValue($dataRow, ['IDCONSIGNEE', 'ID_CONSIGNEE'], 10),
                    'consignee' => $this->getRowValue($dataRow, ['CONSIGNEE', 'NAMA_CONSIGNEE', 'PEMILIK'], 11),
                    'no_bc11' => $noBc11 ?: '-',
                    'tgl_bc11' => $this->getRowValue($dataRow, ['TGLBC11', 'TGL_BC11'], 13),
                    'no_pos_bc11' => $this->getRowValue($dataRow, ['NOPOSBC11', 'NO_POS_BC11'], 14),
                    'jml_satuan' => floatval($this->getRowValue($dataRow, ['JMLSATOAN', 'JMLSATUAN', 'JML_SATUAN', 'JUMLAH', 'QTY'], 16, 0)),
                    'jns_satuan' => $this->getRowValue($dataRow, ['JNSSATUAN', 'JNS_SATUAN', 'SATUAN'], 17, 'KGM'),
                    'kd_dok_inout' => $this->getRowValue($dataRow, ['KDDOKINOUT', 'KD_DOK_INOUT'], 18, '1'),
                    'no_dok_inout' => $noDokInout ?: '-',
                    'tgl_dok_inout' => $this->parseDate($this->getRowValue($dataRow, ['TGLDOKINOUT', 'TGL_DOK_INOUT'], 20)),
                    'wk_inout' => $this->parseDateTime($this->getRowValue($dataRow, ['WKINOUT', 'WK_INOUT'], 21)),
                    'kd_sar_angkut_inout' => $this->getRowValue($dataRow, ['KDSARANGKUTINOUT', 'KD_SAR_ANGKUT_INOUT'], 22, '1'),
                    'pel_muat' => $this->getRowValue($dataRow, ['PELMUAT', 'PEL_MUAT'], 23),
                    'pel_transit' => $this->getRowValue($dataRow, ['PELTRANSIT', 'PEL_TRANSIT'], 24),
                    'pel_bongkar' => $this->getRowValue($dataRow, ['PELBONGKAR', 'PEL_BONGKAR'], 25),
                    'jenis_isi' => 'KIMIA',
                    'satuan' => $this->getRowValue($dataRow, ['JNSSATUAN', 'JNS_SATUAN', 'SATUAN'], 17, 'KGM'),
                    'jumlah_isi' => floatval($this->getRowValue($dataRow, ['JMLSATOAN', 'JMLSATUAN', 'JML_SATUAN', 'JUMLAH', 'QTY'], 16, 0)),
                ];
            }

            $groups = array_values($groupedData);
            if (empty($groups)) {
                return response()->json(['error' => 'Tidak ditemukan data Pemasukan (KD_DOK = 1) yang valid dalam file Excel.'], 422);
            }
            ...
            Log::error("Import Pemasukan Preview Error: " . $e->getMessage());
            return response()->json(['error' => 'Gagal membaca file Excel: ' . $e->getMessage()], 500);
        }
    }

    /**
     * Confirm and Save Excel Import Groups
     */
    public function importConfirm(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => "Berhasil mengimpor {$createdDocuments} Dokumen Master dan {$createdTangki} Detail Tangki Pemasukan.",
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Import Pemasukan Confirm Error: " . $e->getMessage());
            return response()->json(['error' => 'Gagal menyimpan data import: ' . $e->getMessage()], 500);
        }
    }

    /**
     * Destroy a Pemasukan document
     */
    public function destroy(Document $document)
    {
        if ($document->kd_dok !== '1') {
            return back()->with('error', 'Dokumen bukan merupakan Dokumen Pemasukan.');
        }

        $document->delete();

        return redirect()->route('pemasukan.index')->with('success', 'Dokumen Pemasukan berhasil dihapus.');
    }
