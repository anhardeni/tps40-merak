...
export default function JenisSatuanEdit({ item }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        kode_satuan_barang: item.kode_satuan_barang,
        nama_satuan_barang: item.nama_satuan_barang,
    })

    const submit = (e: React.FormEvent) => {
        e.preventDefault()
        put(route('jenis-satuan.update', item.id))
    }

    return (
        <AuthenticatedLayout>
            <Head title="Edit Jenis Satuan" />
