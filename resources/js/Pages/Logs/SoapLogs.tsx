...
export default function SoapLogs({ auth, soapLogs, filters }: SoapLogsProps) {
    ...
    const formatResponseTime = (time: any) => {
        ...
        if (numTime < 1000) {
            return `${numTime.toFixed(1)}ms`
        }
        return `${(numTime / 1000).toFixed(2)}s`
    }
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            <div className="space-y-6 p-4 md:p-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>SOAP API Calls ({soapLogs?.meta?.total || 0})</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {(!soapLogs?.data || soapLogs.data.length === 0) ? (
                                            ...
                                            <tbody>
                                                {(soapLogs?.data || []).map((log) => (
                                                    <tr key={log.id} className="border-b hover:bg-slate-50 dark:hover:bg-slate-900">
                                                        ...
                                                        <td className="p-4">
                                                            <span className={`text-sm ${log.response_time > 2000 ? 'text-red-600' :
                                ...
                                {soapLogs?.links && soapLogs.links.length > 3 && (
                                    <div className="flex items-center justify-between pt-4">
                                        <div className="text-sm text-slate-600 dark:text-slate-400">
                                            Showing {soapLogs?.meta?.from || 0} - {soapLogs?.meta?.to || 0} of {soapLogs?.meta?.total || 0} logs
                                        </div>
